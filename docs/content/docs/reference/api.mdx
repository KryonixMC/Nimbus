...
| `system` | Object | Host specs + live CPU/memory. Added in v0.6.3. Static fields (hostname, OS, CPU model) are read once and cached; `systemCpuLoad`, `processCpuLoad` and `systemMemoryUsedMb` are resampled on every request |
...
| `system` | Object | Static host specs reported at auth time plus live CPU/memory snapshot from the latest heartbeat. Available since v0.6.3 — null for older agents |
