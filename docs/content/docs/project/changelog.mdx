...
- **ApiError-Enum-Konsolidierung — BREAKING** — die lose `ApiErrors`-String-Konstanten sind jetzt in einem typisierten `enum class ApiError(val code: String, val defaultStatus: HttpStatusCode)` zusammengefasst, über den alle Routen einheitlich Fehler-Codes und Default-HTTP-Status beziehen. `object ApiErrors` bleibt als `@Deprecated`-Kompat-Facade erhalten und delegiert intern auf den Enum, sodass bestehende Call-Sites und externe API-Clients unverändert weiterlaufen — die Wire-Strings (`VALIDATION_FAILED`, `SERVICE_NOT_FOUND`, …) sind byte-identisch. Neu strukturiert in Familien: `AUTH_*` (TOTP, Session, Magic-Link, Login-Challenge) und `BACKUP_*` für alle Backup-Modul-Fehler, plus die bisherigen Core-Codes. Entfernt wurde der ungenutzte `INSUFFICIENT_SCOPE`-Code (keinen Nutzer im Repo gehabt); die generischen `NOT_FOUND` und `INVALID_INPUT` sind auf spezifischere typisierte Varianten umgebogen, die Alt-Konstanten existieren aber noch als Wire-String-Mapping für Rückwärtskompatibilität. Auf API-Consumer-Seite keine Änderung nötig.
- **Permission-Node-Umbenennung — BREAKING** — der bisherige Node `nimbus.players` heißt jetzt `nimbus.cloud.players`, passend zur restlichen `nimbus.cloud.*`-Admin-Familie. Die Umstellung wird beim Start automatisch vorgenommen: `PermsModule.enable()` ruft nach `permissionManager.init()` die neue `PermissionManager.renamePermissionInAllGroups(old, new)`-API, die idempotent über alle Gruppen hinweg arbeitet — global, negiert, context-scoped und negiert-context-scoped. Duplikate werden dedupliziert, Contexts zusammengeführt. Operatoren, die die Auto-Migration aussetzen müssen (z. B. für Audits oder eigene Migrations-Skripte), können per `[permissions] skip_node_migrations = true` in `nimbus.toml` deaktivieren. Neues vollständiges Permission-Register in `docs/reference/permissions.mdx` dokumentiert alle drei Familien: `nimbus.cloud.*` (Admin/Operations), `nimbus.dashboard.*` (Web-UI) und `nimbus.<module>.<behavior>` (In-Game-Gameplay).
- **SDK Public Surface eingefroren** — die stabile API-Oberfläche von `plugins/sdk` ist jetzt dokumentiert und gegen versehentliche Änderungen gesichert. Klassen ohne `@ApiStatus.Internal` bilden den unterstützten API-Kontrakt — Breaking Changes daran erfolgen nur in Major-Versions mit dokumentiertem Migration-Guide; mit `@ApiStatus.Internal` markierte Klassen sind Implementierungsdetails und dürfen zwischen Minor-Versions ohne Vorwarnung wechseln. Erster markierter Kandidat: `EventRegistry` (interner Dispatch-Helper für `NimbusEventStream`, nie für externe Consumer gedacht). `NimbusSdkPlugin` bleibt public, sein Javadoc klärt aber explizit, dass die Klasse ein Server-Entry-Point ist und nicht erweitert, subclassed oder instanziiert werden darf — SDK-Zugriff erfolgt über `Nimbus`. Neue `package-info.java`-Dateien in `sdk/`, `sdk/compat/` und `sdk/event/` dokumentieren den Kontrakt direkt an der Paketgrenze. Ein neuer Regression-Test `SdkPublicSurfaceTest` sperrt sowohl die stabile Liste als auch die Internal-Liste per Class-File-Scan gegen unbeabsichtigte Umklassifizierungen.
...
- **Permission-Node `nimbus.players` → `nimbus.cloud.players`:** Operatoren müssen nichts tun — die Auto-Migration fügt beim ersten Start von Nimbus 0.13.0 allen Gruppen, die bereits den alten Node haben, zusätzlich den neuen Node hinzu (unter Erhalt von Negierungen und Contexts). Der alte Node bleibt bis Nimbus 1.0 funktional, in 1.0 wird er entfernt — bis dahin sollten Gruppen-Exporte und externe Tooling-Skripte auf den neuen Namen umgestellt werden.
