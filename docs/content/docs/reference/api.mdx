...
<Callout type="info">
  **Removed / renamed in 0.13:** `INSUFFICIENT_SCOPE` was unused and has been removed. `INVALID_INPUT` and `NOT_FOUND` are deprecated — every endpoint now returns a domain-specific code (e.g. `BACKUP_NOT_FOUND`, `PERMISSION_GROUP_NOT_FOUND`). Until 0.14 the old string constants still resolve via a compat facade (`INVALID_INPUT` silently maps to `VALIDATION_FAILED`).
</Callout>
